// Copied from https://github.com/microsoft/create-github-app-token-via-key-vault/blob/main/src/api.ts
...
export function createGitHubAppAuth(options: GitHubAppAuthOptions): GitHubAppAuth {
    ...
    async function discoverInstallation(target: InstallationTarget): Promise<{ id: number; appSlug: string; }> {
        ...
        switch (target.type) {
            ...
            case "repository":
                installation = await requestJson(
                    `${githubApiUrl}/repos/${target.owner}/${
                        assertValue(target.repositories[0], "repository is required")
                    }/installation`,
                    { headers: githubHeaders(jwt) },
                    "Could not discover GitHub App installation ID",
                );
    ...
    async function getInstallationToken(options: GetInstallationTokenOptions): Promise<InstallationToken> {
        const target = resolveInstallationTarget(options, defaultOwner);
        ...
        return retryTransient(async () => {
            ...
            const result: InstallationToken = {
                ...
                repositories,
                permissions: isRecord(token) && isRecord(token["permissions"])
                    ? token["permissions"]
                    : (permissions ?? {}),
            };
    ...
    async function getToken(options: GetInstallationTokenOptions): Promise<string> {
        return (await getInstallationToken(options)).token;
    }
